(function (global, factory) {
  ...
  var isPromise = function isPromise(type) {
    ...
    callController = function callController(controller, context) {
      var ret = typeof controller === "function" ? controller(context) : identity(context);
      if (!isPromise(ret)) {
      ...
      resolve: function resolve(path, action) {
        ...
        if (routeInfo) {
          ...
          if (this.current) {
            ...
            if (runtimePath === path) {
              var _ret = callController(routeInfo.controller, _objectSpread(_objectSpread({}, context), {}, {
                route: _objectSpread(_objectSpread({}, this.current), {}, {
                  params: routeInfo.params,
                  state: this.state
                })
              }));
              return _ret.then(function (retVal) {
          ...
          var ret = callController(controller, ctx);
  ...
  function createRouter() {
    ...
    return Object.create(RouterProto, {
      ...
      options: {
        value: _objectSpread(_objectSpread({}, routerDefaults), options)
      },
      emitter: {
